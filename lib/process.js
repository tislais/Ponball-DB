...
import { getContributors, getFiles, getImages } from './processUtils.js';
...
export default async function processPage(html) {
  ...
  rows.forEach((row) => {
    ...
    switch (label) {
      ...
      case 'Multimedia Files:':
        filesRow++;
        break;
      case 'Images: (click to zoom)':
        images = getImages(row.querySelectorAll('td')[1]);
        break;
      default:
        true;
    }
  ...
  // Shameful bug fixes. Please revisit.
