...
export default async function processPage(html) {
  ...
  rows.forEach(row => {
    ...
    if (label === 'Manufacturer:' && regex.test(manufacturerDateRange)) {
      console.log(`\x1b[35m%s\x1b[0m`,'manufacturerDateRange: ', manufacturerDateRange);
      
      console.log(`\x1b[35m%s\x1b[0m`,'manufacturerDateRange: ', regex.test(manufacturerDateRange));
